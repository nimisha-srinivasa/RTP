...
extern void stemContent(struct stemmer * z, const char* content);
...
/* cons(z, i) is T_TRUE <=> b[i] is a consonant. ('b' means 'z->b', but here
   and below we drop 'z->' in comments.
*/

static int cons(struct stemmer * z, int i)
{  switch (z->b[i])
   {  case 'a': case 'e': case 'i': case 'o': case 'u': return T_FALSE;
      case 'y': return (i == 0) ? T_TRUE : !cons(z, i - 1);
      default: return T_TRUE;
   }
}
...
/* vowelinstem(z) is T_TRUE <=> 0,...j contains a vowel */

static int vowelinstem(struct stemmer * z)
{
   int j = z->j;
   int i; for (i = 0; i <= j; i++) if (! cons(z, i)) return T_TRUE;
   return T_FALSE;
}

/* doublec(z, j) is T_TRUE <=> j,(j-1) contain a double consonant. */

static int doublec(struct stemmer * z, int j)
{
   char * b = z->b;
   if (j < 1) return T_FALSE;
   if (b[j] != b[j - 1]) return T_FALSE;
   return cons(z, j);
}

/* cvc(z, i) is T_TRUE <=> i-2,i-1,i has the form consonant - vowel - consonant
...
{  if (i < 2 || !cons(z, i) || cons(z, i - 1) || !cons(z, i - 2)) return T_FALSE;
   {  int ch = z->b[i];
      if (ch  == 'w' || ch == 'x' || ch == 'y') return T_FALSE;
   }
   return T_TRUE;
}

/* ends(z, s) is T_TRUE <=> 0,...k ends with the string s. */

static int ends(struct stemmer * z, char * s)
{  int length = s[0];
   char * b = z->b;
   int k = z->k;
   if (s[length] != b[k]) return T_FALSE; /* tiny speed-up */
   if (length > k + 1) return T_FALSE;
   if (memcmp(b + k - length + 1, s + 1, length) != 0) return T_FALSE;
   z->j = k-length;
   return T_TRUE;
}
...
extern void stemContent(struct stemmer * z, const char* content) //stems the content and outputs to stdout
{  
  int j=0;
